...
export function KitMlbModal({ kit, onFechar }: Props) {
  ...
  useEffect(() => {
    mlbService.listarPorKit(kit.id).then((data: MlbEntry[]) => {
      setLista(data.map(({ id: _id, itemId: _iid, kitId: _kid, ...rest }) => rest));
      setLoading(false);
  ...
  const salvar = async () => {
    ...
    setMensagem(null);
    try {
      const resultado = await mlbService.salvarPorKit(kit.id, lista);
      setLista(resultado.map(({ id: _id, itemId: _iid, kitId: _kid, ...rest }) => rest));
    ...
    >
      <div className="bg-white flex flex-col" style={{ width: "960px", maxWidth: "95vw", maxHeight: "90vh" }}>
        ...
        <div className="flex items-center justify-between px-4 py-3 text-white shrink-0" style={{ backgroundColor: "#22252A" }}>
          ...
          <button onClick={onFechar} className="text-white opacity-60 hover:opacity-100 text-lg leading-none">✕</button>
        </div>
        ...
        {mostrarImport && (
          <div className="px-4 py-3 bg-[#f5f5f0] border-b border-gray-300 shrink-0 space-y-2">
            <p className="text-[11px] text-gray-600">Cole o texto bruto do campo MLB. Códigos de <strong>exatamente 10 dígitos</strong> serão importados; o texto após cada código vira o campo Modelo.</p>
            <textarea
              value={rawImport}
        ...
        <div className="overflow-auto flex-1">
          {loading ? (
            <div className="text-center py-8 text-[11px] text-gray-400">Carregando...</div>
          ) : (
        ...
        <div className="flex items-center justify-end gap-2 px-4 py-3 bg-[#ececec] border-t border-gray-300 shrink-0">
          <button onClick={onFechar} className="text-[11px] px-4 py-1.5 border border-gray-400 hover:bg-gray-200 transition-colors">Fechar</button>
          <button onClick={salvar} disabled={salvando} className="text-[11px] px-4 py-1.5 font-semibold text-white transition-colors disabled:opacity-50" style={{ backgroundColor: "#22252A" }}>
