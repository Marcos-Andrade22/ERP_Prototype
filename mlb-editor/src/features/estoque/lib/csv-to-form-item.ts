...
export const rawRowToFormItem = (row: RawRow, idx: number): EstoqueItem => {
  ...
  return {
    ...
    montadora: col(row, COLUMN_MAP.montadora),
    aplicacoes: col(row, COLUMN_MAP.aplicacoes),
    ...
    substituicaoTributariaValor: col(row, COLUMN_MAP.substituicaoTributariaValor),
    substituicaoTributariaTipo: "percent",
    lucroTipo: "percent",
    lucroValor: 0,
    acrescimoPercent: 0,
    frete: "",
    taxaClienteOficina: 0,
    observacoesGerais: col(row, COLUMN_MAP.observacoesGerais),
    itensSimilaresCompactibilidade: col(row, COLUMN_MAP.itensSimilaresCompactibilidade),
